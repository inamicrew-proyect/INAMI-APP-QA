    ...
    const fetchRoles = async () => {
      try {
        ...
        if (response.ok && result.roles) {
          ...
          setRoles(rolesList)
          // Si no hay roles cargados, establecer un valor por defecto
          if (rolesList.length > 0 && !rolesList.find((r: { value: string; label: string }) => r.value === formData.role)) {
            setFormData(prev => ({ ...prev, role: rolesList[0].value }))
          }
        }
      } catch (err) {
